...
def enviar_email(email, senha, destinatario):
    try:
...
def autenticar_email(entry_email, entry_senha, root, destinatario):
    ...
    try:
        ...
        enviar_email(email, senha, destinatario)
    except Exception as e:
        with open("error.log", "w") as f:
...
def abrir_janela_envio_email(email, senha, root):
    ...
    btn_enviar = tk.Button(
        ...
        command=lambda: enviar_email(
            email, senha, entry_destinatario.get(), root, None
        ),
    )  # Aqui passamos None como google_auth_response, pois ainda não autenticamos novamente
